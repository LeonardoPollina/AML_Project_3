...
xticks([6 6.5 7 7.5])
xticklabels({'6','6.5','7','7.5'})
yticks([6 6.5 7 7.5])
yticklabels({'6','6.5','7','7.5'})
xlim([6 7.5])
ylim([6 7.5])
...
%axis equal
%axis tight
xlabel('dx before stride','FontWeight','bold','FontSize',24,...
    'FontName','Times New Roman')
ylabel('dx after stride','FontWeight','bold','FontSize',24,...
    'FontName','Times New Roman')
h=legend('Real input-output function','Periodicity Line (dx_{in}=dx_{out})')
set(h,'FontSize',16)
...
subplot(2,1,2);
plot(dx_in(1:end-1),diff(dx_out)./diff(dx_in), 'color', 'b','LineWidth', 2)
ax = gca;
ax.FontSize = 20;
hold on
grid on
...
ylabel('Eigenvalue','FontWeight','bold','FontSize',24,...
        'FontName','Times New Roman')
xlim([6 7.5])
...
h = legend('Eigenvalues of dx return map', 'Stability border: Eigenvalue = 1')
set(h, 'FontSize',16)
