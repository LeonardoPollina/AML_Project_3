...
for i = 1 : n_k
    ...
    for j = 1 : n_angAtt
        ...
        new_score = implemented_alternative_score_function(y_var);
        
        sim_num = sim_num + 1;
        fprintf('\n')    
        disp(['    ========== Simulation No.: ',num2str(sim_num), ' out of ' ,num2str(all_nodes), ' ============'])          
        disp([' ===> angAtt: [',num2str(angAtt(j)), '] k: [', num2str(K_sp(i)), '] ==> n_successful_steps: [',num2str(new_score), ']'])         
        disp('=======================================================')

        all_Ksps(i,j)     = K_sp(i); % to dimensionalize *m_0*g/l_0/1000
        all_angAtts(i,j)  = angAtt(j);
        all_steps(i,j)    = new_score;
